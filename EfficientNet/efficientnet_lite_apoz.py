...
class MBConvBlock(nn.Module):
    ...
    def forward(self, x, drop_connect_rate=None):
        ...
        identity = x
        expand_conv_output = None
        if self.expand_ratio != 1:
            expand_conv_output = self._relu(self._bn0(self._expand_conv(x)))
            
        if expand_conv_output is None:
            expand_conv_output_2 = x
        else:
            expand_conv_output_2 = expand_conv_output
            
        depthwise_output = self._relu(self._bn1(self._depthwise_conv(expand_conv_output_2)))
        x = depthwise_output
        # Squeeze and Excitation
        se_reduce_output = None
        se_expand_output = None
        if self.has_se:
            x_squeezed = F.adaptive_avg_pool2d(x, 1)
            se_reduce_output = self._relu(self._se_reduce(x_squeezed))
            se_expand_output = self._se_expand(se_reduce_output)
            x = torch.sigmoid(se_expand_output) * x
            
        project_conv_output = self._project_conv(x)
        x = self._bn2(project_conv_output)
        ...
        return (expand_conv_output, depthwise_output, project_conv_output, se_reduce_output, se_expand_output), x
...
class EfficientNetLite(nn.Module):
    ...
    def forward(self, x):
        ...
        x = stem_output
        idx = 0
        for idx3, stage in enumerate(self.blocks):
            for idx2, block in enumerate(stage):
                drop_connect_rate = self.drop_connect_rate
                if drop_connect_rate:
                    drop_connect_rate *= float(idx) / len(self.blocks)
                outputs, x = block(x, drop_connect_rate)
                outputs_blocks.extend([i for i in outputs])
                idx +=1
        head_output = self.head(x)
        x = self.avgpool(head_output)
        x = torch.flatten(x, 1)
        if self.dropout is not None:
            x = self.dropout(x)
        x = self.fc(x)

        return (outputs_blocks, stem_output, head_output), x
