...
import sys
import dataset
import network_apoz_teacher_student
import network_teacher_student
import dataset_apoz
import train_test
...
model = network_apoz_teacher_student.RecycleNetwork(num_classes=5).to(device)
model.load_state_dict(torch.load('recycle_net_l1_0_sum.pt'))
model.eval()
...
model = network_teacher_student.RecycleNetwork(num_classes=5).to(device)
model.load_state_dict(torch.load('recycle_net_l1_0_sum.pt'))
model.train()
