...
def train_network(model, num_epochs, optimizer, loss_function_classify, loss_function_mse, loss_weight_mse,  trainloader, validloader, device, patience=10, path_to_model='recycle_net', scheduler=None):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for batch in trainloader:
            ...
            softmax = batch[2].to(device)
            # print(softmax)
            outputs = model(images)
            # print(tempature_softmax(outputs, tempature=2.0))
            loss_classify = loss_function_classify(outputs, labels)
            loss_mse = loss_function_mse(tempature_softmax(outputs, tempature=2.0), softmax)
            # print(loss_mse)
            #Calculate the gradients for parameters (backprop)
            loss = loss_mse + loss_classify
            loss.backward()
            #Update the values of the parameter using the optimizer
            optimizer.step()
            train_loss.append(loss_classify.item())
        ...
        with torch.no_grad():
            for batch in validloader:
                ...
                loss_mse = loss_function_mse(tempature_softmax(outputs, tempature=2.0), softmax)
                loss = loss_mse + loss_classify
                valid_loss.append(loss_classify.item())
                valid_loss_mse.append(loss_mse.item())
        ...
        print('Epoch: {}, Training Loss Classify: {:4f}, Training Loss MSE: {:4f}, Training Loss Total: {:4f}, Validation Loss: {:4f}, Validation Loss MSE: {:4f}, Validation Loss Total: {:4f}, Training Accuracy: {:4f}, Validation Accuracy: {:4f}'.format(epoch, np.mean(train_loss), np.mean(train_loss_mse), np.mean(total_train_loss), np.mean(valid_loss), np.mean(valid_loss_mse), np.mean(total_valid_loss), num_correct_train/num_examples_train, num_correct_valid/num_examples_valid))
        ...
        early_stop_loss = np.mean(total_valid_loss)
